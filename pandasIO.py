...
print df.head()
df.to_csv('newcsv3.csv')# save to a csv fie

# may you don want the name in the file so 
# pandas can manage this

df.to_csv('newcsv4.csv',header = False)# save to a csv fie

# we can do all the above in one command line
df = pd.read_csv('newcsv4.csv',names = ['Date','Bolivia exports to Brazil'], index_col=0)# save to a csv fie
print df.head() # doesn't affect the data

#######
# converting to other formats
df.to_html('bobraexport.html')

df = pd.read_csv('newcsv4.csv', names=['Date','Exports'],index_col=0)
print df.head()
